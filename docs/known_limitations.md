...
- `BLE_PACKET_RECEIVED` dan `ACK_RECEIVED` memakai `event_key` unik per
  `observation_id`, sehingga retry protocol dapat mengisi event fisik yang
  hilang setelah crash tetapi tidak menambah sample ganda. Observasi tanpa
  `observation_id` tetap mengikuti perilaku legacy dan dicatat sebagai receive
  normal.
